...
from news_data.fetcher.blacklist import blacklist
...
from news_data.fetcher.newsapi import get_base_domain

og_db = Database(article_collection="all_articles_layered_archive_2")
og_articles_db = ArticleDB(og_db)

new_db = Database(article_collection="all_articles_layered")
new_articles_db = ArticleDB(new_db)

new_graph_handler = Neo4jGraphHandler("neo4j://127.0.0.1:7687", "neo4j", "PWD", "aura-articles-labelled-2")
...
for old_article in all_old_articles:
    ...
    if get_base_domain(old_article["url"]) in blacklist:
        pruned += 1
        continue
